...
/**
 * {@linkplain SessionExpirationService_SessionMetadata_Get} is a singleton
 * class that resets the max idle time of all the specified session relevant
 * entries from the pattern matching maps by reading (getting) entries.
 * 
 * @author dpark
 *
 */
public class SessionExpirationService_SessionMetadata_Get implements SessionExpirationServiceConfiguration {

	private final static SessionExpirationService_SessionMetadata_Get expirationService = new SessionExpirationService_SessionMetadata_Get();
	...
	public final static SessionExpirationService_SessionMetadata_Get getExpirationService() {
		return expirationService;
	}

	private SessionExpirationService_SessionMetadata_Get() {
	}
	...
	synchronized void initialize(Properties properties) {
		...
		tag = properties.getProperty(PROPERTY_TAG, SessionExpirationService_SessionMetadata_Get.class.getSimpleName());
		logPrefix = tag + ": ";
		...
		consumerThread = new Thread(consumer, SessionExpirationService_SessionMetadata_Get.class.getSimpleName());
		...
		if (isJmxEnabled) {
			...
			try {
				...
				if (isJmxUseHazelcastObjectName) {
					...
					type = SessionExpirationService_SessionMetadata_Get.class.getSimpleName();
				}
				objectName = new ObjectName(
						header + ":name=" + SessionExpirationService_SessionMetadata_Get.class.getSimpleName()
	...
	/**
	 * Resets max-idle-time-seconds of all the entries from the configured maps that have
	 * the matching session.
	 * 
	 * @param sessionMapName The expiration originated map.
	 * @param key            Key object containing the session ID.
	 */
	public void reset(SessionMetadata sm) {
		...
		public void run() {
			while (shouldRun) {
				...
				try {
					...
					// Invoke get() to reset the idle timeout.
	 ...
	 * {@linkplain SessionExpirationService_SessionMetadata_Get} enqueues
	 ...
	 * {@linkplain SessionExpirationService_SessionMetadata_Get} initialization
	 * time.
	 * 
	 * @author dpark
	 *
